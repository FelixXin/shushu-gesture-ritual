...
一个与原奇门知识库隔离的独立项目。使用浏览器摄像头与 MediaPipe 手势识别，配合原创 14 层 Three.js 罗盘完成六次起卦，并显示本卦、动爻与变卦。

## 已实现

- 浏览器摄像头授权与实时手部关键点叠加。
- `Pointing_Up` 食指唤醒、`Closed_Fist` 平面/3D 切换、`Open_Palm` 调整空间阵盘、`Victory` V 手势生成一爻。
- 天池太极至周天刻度的 14 层同心罗盘；六爻完成后自动展开并锁定本卦扇区。
